...
class Intrinsic:
    ...
    def __init__(self, context_type: str, seed: int, summary_output_path: str, save_path: str, dataset: str):
        ...
        self.file_output_path = f"{self.save_path}/{self.dataset}/intrinsic_{self.context_type}_{self.seed}.parquet"
        logger.info(f"Saving intrinsic results to {self.file_output_path}")
    ...
    def process_chunk_torch_vectorized(self, chunk, num_samples=1000, torch_seed=42, device="cpu"):
        ...
        if self.context_type == "negative":
            sentences_to_choose = range(100, 200)
        elif self.context_type in ["positive", "combined"]:
        ...
        for word, step in word_step_pairs:
            ...
            q_c = torch.tensor(model_word_step_df["p_c"].values, dtype=torch.float32, device=device)
            ...
            sampled_c = q_c[sampled_indexes]

            log_val = torch.log2(sampled_q_w_given_c)

            if self.context_type in ["positive", "negative"]:
                denominator = torch.sum(sampled_q_w_given_c * sampled_c, dim=1)
                weight = (sampled_q_w_given_c * sampled_c) / denominator[:, None]
            elif self.context_type == "combined":
                weight = sampled_c

            bootstrap_intrinsic = -torch.sum(weight * log_val, dim=1)

            mean_intrinsic = torch.mean(bootstrap_intrinsic).item()
            std_intrinsic = torch.std(bootstrap_intrinsic).item()

            results.append(
                {
                    "word": word,
                    "step": step,
                    "mean_value": mean_intrinsic,
                    "std_value": std_intrinsic,
                }
            )

        return results

    def get_intrinsic(self, num_samples=1000) -> None:
        df_step_intrinsic = pd.DataFrame(columns=["word", "step", "mean_value", "std_value"])
        df_step_intrinsic = df_step_intrinsic.astype(
        ...
        df_step_intrinsic = pd.DataFrame(flattened_results)

        df_step_intrinsic.to_parquet(self.file_output_path, index=False)
